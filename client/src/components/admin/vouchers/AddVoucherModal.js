...
import { createVoucher, getAllVouchers } from "./FetchApi";
...
const AddVoucherModal = (props) => {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      let responseData = await createVoucher(voucherData);
      if (responseData.success) {
        message.success(responseData.message || "Tạo voucher thành công");
        fetchData();
        dispatch({ type: "addVoucherModal", payload: false });
        // Reset form
        setFdata({
          code: "",
          name: "",
      ...
      console.log(error);
      message.error("Có lỗi xảy ra khi tạo voucher");
      dispatch({ type: "loading", payload: false });
    }
  };
  ...
  return (
    <Fragment>
      {/* Black Overlay */}
      <div
        onClick={(e) => dispatch({ type: "addVoucherModal", payload: false })}
        className={`${
          data.addVoucherModal ? "" : "hidden"
