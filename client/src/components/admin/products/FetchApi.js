...
export const createProduct = async ({
  ...
  formData.append("pQuantity", pQuantity || 0);
  formData.append("pPrice", 0); // Giá sẽ được tính từ variants
  formData.append("pOffer", pOffer);
  // Append variants as JSON string - BẮT BUỘC
...
export const editProduct = async (product) => {
  ...
  formData.append("pQuantity", product.pQuantity || 0);
  formData.append("pPrice", 0); // Giá sẽ được tính từ variants
  formData.append("pOffer", product.pOffer);
  formData.append("pImages", product.pImages);
  // Append variants as JSON string - BẮT BUỘC
  if (product.pVariants && product.pVariants.length > 0) {
    formData.append("pVariants", JSON.stringify(product.pVariants));
  }
